<?php
/**
 * Various functions required for drivewyze-foundation to work properly
 *
 * @package Drivewyze
 */
 ...
 */
if ( ! class_exists( 'Drivewyze_Navwalker' ) ) :
	/**
	 * Drivewyze_Navwalker
	 */
	class Drivewyze_Navwalker extends Walker_Nav_Menu {
		/**
		 * start_lvl
		 *
	...
	if ( ! class_exists( 'Drivewyze_Mobile_Navwalker' ) ) :
		/**
		 * Drivewyze_Mobile_Navwalker
		 */
		class Drivewyze_Mobile_Navwalker extends Walker_Nav_Menu {
			/**
			 * start_lvl
			 *
			 * @param string  $output output
 ...
 */
if ( ! function_exists( 'drivewyze_menu_fallback' ) ) :
	/**
	 * drivewyze_menu_fallback
	 *
	 * @return void
	 */
	function drivewyze_menu_fallback() {
...
if ( ! function_exists( 'drivewyze_active_nav_class' ) ) :
	/**
	 * drivewyze_active_nav_class
	 *
	 * @param array $classes classes
	 * @param mixed $item item
	 * @return array $classes classes
	 */
	function drivewyze_active_nav_class( $classes, $item ) {
		if ( 1 === $item->current || true === $item->current_item_ancestor ) {
			$classes[] = 'active';
		}
		return $classes;
	}
	add_filter( 'nav_menu_css_class', 'drivewyze_active_nav_class', 10, 2 );
endif;

// 3 - Pagination: {@link https://codex.wordpress.org/Pagination}
if ( ! function_exists( 'drivewyze_pagination' ) ) :
	/**
	 * drivewyze_pagination
	 *
	 * @return void
	 */
	function drivewyze_pagination() {
		global $wp_query;
 ...
 */
if ( ! function_exists( 'drivewyze_active_list_pages_class' ) ) :
	/**
	 * drivewyze_active_list_pages_class
	 *
	 * @param mixed $input input
	 */
	function drivewyze_active_list_pages_class( $input ) {
		$pattern = '/current_page_item/';
		$replace = 'current_page_item active';
		$output  = preg_replace( $pattern, $replace, $input );
		return $output;
	}
	add_filter( 'wp_list_pages', 'drivewyze_active_list_pages_class', 10, 2 );
endif;

// 4 - Comments tree: @link {https://codex.wordpress.org/Function_Reference/Walker_Comment}
if ( ! class_exists( 'Drivewyze_Comments' ) ) :
	/**
	 * Drivewyze_Comments
	 */
	class Drivewyze_Comments extends Walker_Comment {
		/**
		 * Tree type.
		 *
		 * @var string
